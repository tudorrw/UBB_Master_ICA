...
def load_conf(conf_path: str, remove_output_setting: bool=False) -> dict:
    """
    Loads the configuration from provided YAML file.
    :param conf_path: path to the configuration file
    :param remove_output_setting: remove any output settings that may be pre-stored in the config, revert to default
    :return: configuration loaded as dict
    """
    with open(conf_path, 'rt') as in_fd:
        conf = yaml.load(in_fd, Loader=yaml.FullLoader)
        if remove_output_setting:
            if 'output_stream_type' in conf:
                del conf['output_stream_type']
            if 'output_file' in conf:
                del conf['output_file']
    if not conf:
...
class DialTaskFormatter(LogFormatter):
    def __init__(self, path_prefix, *args, **kwargs):
        self.path_prefix = path_prefix
        super(DialTaskFormatter, self).__init__(*args, **kwargs)

    def format(self, record):
        record.relpath = record.pathname.replace(self.path_prefix, '')
        formatted = super(DialTaskFormatter, self).format(record)
        return formatted
