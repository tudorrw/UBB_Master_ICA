...
class ConversationHandler(object):
    ...
    def __init__(self, config, logger=None, should_continue=None, user_stream=None):
        ...
        if user_stream is not None:
            ...
            if 'user_stream_type' not in self.config:
                self.config['user_stream_type'] = 'dialtask.input.text.ConsoleInput'
            try:
        ...
        if 'output_stream_type' not in self.config:
            self.config['output_stream_type'] = 'dialtask.output.text.ConsoleOutput'
        try:
    ...
    def run_dialogue(self, dial: Dialogue):
        ...
        while not eod:
            ...
            self.output_stream(system_response, dial)
    ...
    def _load_components(self):
        ...
        if 'components' not in self.config:
            self.logger.warning('The "components" entry has not been found in the configuration, the components pipeline will be empty!')
            return
